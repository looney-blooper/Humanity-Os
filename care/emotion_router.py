...
class EmotionResponse(BaseModel):
    facial_emotion: Optional[str] = Field(None, description="Detected facial emotion (if image provided)")
    facial_confidence: Optional[float] = Field(None, description="Confidence score for facial emotion (if image provided)")
...
async def analyze_emotion(request: EmotionRequest):
    """
    Analyze emotion from optional base64 encoded image and assess mental state from Q&A responses.
    
    Args:
        request: EmotionRequest containing optional base64 image and list of Q&A pairs
    ...
    try:
        ...
        # 1. Process facial emotion ONLY if image is provided
        ...
        # Include facial emotion context only if available
        facial_context = ""
        if facial_emotion is not None:
            facial_context = f"""**Facial Emotion Detected:** {facial_emotion} (confidence: {facial_confidence}%)

"""
        
        assessment_prompt = f"""You are a compassionate mental health assistant. Analyze the following information about a user:

{facial_context}**User's Responses to Questions:**
{qa_text}
...
Consider:
{'- Consistency between facial emotion and written responses' if facial_emotion else '- Focus on written responses and language patterns'}
